...
int main()
{
    ...
    vector<char> fold_axii;
    vector<int> fold_coords;
    ...
    while(getline(f, str))
    {
        if (str.size() == 0)
        {
            while(getline(f, str))
            {
                fold_axii.push_back(str[string("fold along ").size()]);
                fold_coords.push_back(stoi(str.substr(string("fold along ").size() + 2, str.size())));
            }
            break;
        }
    ...
    auto paper_folded = paper;
    for (int i = 0; i < fold_axii.size(); i++)
    {
        paper_folded = fold(paper_folded, fold_axii[i], fold_coords[i]);
    }
